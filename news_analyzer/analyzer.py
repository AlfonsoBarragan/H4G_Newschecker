...
def converse_to_tokens(words_to_tokenize):
    ...
    relative_pronouns_list                  = ['that', 'which', 'who', 'whom',
            ...
            for simil in similarity_list:
                if bigger_list == synsets_list and has_meaning == False:
                    if smaller_similarity in simil:
                        meaning_def = wn.synsets(word, lang='spa')[similarity_list.index(simil)]
...
def write_lexical_analysis_result(descriptions_tokens_sequence):
    ...
    for description in descriptions_tokens_sequence:
        for phrases in description:
            ...
            for word in phrases:
                print(word.word)
                print("{}: {}".format(word.word, word.lex_class))
...
def analyze_exec():
    ...
    news_text = load_news_text(route_database)

    return lexical_analysis(traduced_descriptions)

def mix_tokens_with_database(route_database, route_tokens_sequence):
    dataframe_database          = load_news_text(route_database)
    dataframe_tokens_sequence   = lexical_analysis(read_traduced_descriptions_file()) 

    print(dataframe_database)
    print(len(dataframe_tokens_sequence))
